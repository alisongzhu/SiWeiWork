...
#include <tcp.h> // required for TCP keepalive
...
//				setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (char *) &optionReuseAddr, sizeof(int));
...
TcpSocket_create()
{
	...
	int optionReuseAddr = 1;
		setsockopt(self->fd, IPPROTO_TCP, TCP_NODELAY, (char *) &optionReuseAddr, sizeof(int));
