...
const struct eth_addr ethbridge = {{0x01,0x80,0xc2,0x00,0x00,0x00}};
...
#else
static void StpCallback_SetPortState(const struct STP_BRIDGE* bridge, unsigned int portIndex, unsigned int treeIndex, unsigned int state, unsigned int timestamp)
{
	uint32_t reg = 0;  
  uint32_t  PortPhy = PortIndexMapToPhy(portIndex);
...
static void* StpCallback_TransmitGetBuffer (const struct STP_BRIDGE* bridge, unsigned int portIndex, unsigned int bpduSize, unsigned int timestamp)
{
  ...
  vlan->prio_vid = lwip_htons((u16_t)((1<< (PortIndexMapToPhy(portIndex)+8) )|0x4021));//speical tag
...
static void StpCallback_FlushFdb (const struct STP_BRIDGE* bridge, unsigned int portIndex, unsigned int treeIndex, enum STP_FLUSH_FDB_TYPE flushType)
{
  ...
  {
    //clear address table
//    HAL_ETH_ReadPHYRegister(&heth, 0x11,&reg);
//    reg = 1<<PortIndexMapToPhy(portIndex);
    HAL_ETH_WritePHYRegister(&heth, 0x11,1<<PortIndexMapToPhy(portIndex));
    reg = (reg& ~0x1c) |0x10;
    HAL_ETH_WritePHYRegister(&heth, 0x10,reg);
  }
  
//  if(flushType == STP_FLUSH_FDB_TYPE_IMMEDIATE)
//  {
//    //delay 4.5ms
//    do
//    {
...
{
	HAL_UART_Transmit(&huart5, (uint8_t *)nullTerminatedString, stringLength,0xffff);
}
...
{
  uint32_t PortPhy = 0;
  if(PortIndex == PORT_FX_1) PortPhy = 1;
  if(PortIndex == PORT_FX_2) PortPhy = 2;
  if(PortIndex == PORT_RJ45) PortPhy = 3;
  if(PortIndex == PORT_CPU)  PortPhy = 5;
  return PortPhy;
}
uint32_t PortPhyMapToIndex(uint32_t PortPhy)
{
  uint32_t PortIndex = 0;
  if(PortPhy == 1) PortIndex = PORT_FX_1;
  if(PortPhy == 2) PortIndex = PORT_FX_2;
  if(PortPhy == 3) PortIndex = PORT_RJ45;
  if(PortPhy == 5) PortIndex = PORT_CPU;
  return PortIndex;
}
...
void stp_init(void)
{
  ...
  STP_SetBridgePriority (bridge, 0, 0x9000, stp_timestamp);
	...
	STP_SetPortAdminPointToPointMAC (bridge, PORT_FX_1, STP_ADMIN_P2P_FORCE_TRUE, stp_timestamp);
	STP_SetPortAdminPointToPointMAC (bridge, PORT_FX_2, STP_ADMIN_P2P_FORCE_TRUE, stp_timestamp);
  STP_SetPortAdminPointToPointMAC (bridge, PORT_RJ45, STP_ADMIN_P2P_FORCE_TRUE, stp_timestamp);
	STP_SetPortAdminPointToPointMAC (bridge, PORT_CPU,  STP_ADMIN_P2P_FORCE_TRUE, stp_timestamp);
