...
static void RS485Port1_init(void);
static void RS485Port2_init(void);
static void vTaskModbusSlave(void const * argument);
static void vTaskModbusMaster(void const * argument);
...
static void RS485Port1_init(void)
{
	...
	switch(opMode)
	{
		...
		case COM_OPMODE_SLAVE_RTU:
			mode = MB_RTU;
			status = xTaskCreate((TaskFunction_t)vTaskModbusSlave,"MsRTU",100,&mode,3,&(RS485Network[0].handler));
			break;
#endif
...
//			status = xTaskCreate((TaskFunction_t)vTaskDTU,"DTU",100,NULL,3,&(RS485Network[1].handler));
		...
		case COM_OPMODE_MASTER_RTU:
			mode = MB_RTU;
			status = xTaskCreate((TaskFunction_t)vTaskModbusMaster,"MmRTU",100,&mode,3,&(RS485Network[1].handler));
			break;
#endif
...
static void vTaskModbusSlave(void const * argument)
{
	...
	RS485Network[0].eStatus = eMBInit(*mode, env.ComEnv[0].address, 1, env.ComEnv[0].baudrate,  (eMBParity)env.ComEnv[0].eParity);
...
void vTaskModbusMaster(void const * argument)
{
	 RS485Network[1].eStatus = MB_ENOERR;
    eMBMode* mode = (eMBMode*)argument;

    RS485Network[1].eStatus  = eMBMasterInit(*mode, 2,env.ComEnv[1].baudrate,(eMBParity)env.ComEnv[1].eParity);
