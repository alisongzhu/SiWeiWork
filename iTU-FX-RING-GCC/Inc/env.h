...
struct sSystemEnv
{
  ...
  uint32_t fw_iTU_size;
  uint32_t fw_iTU_crc;
  uint32_t fw_extend_size;
  uint32_t fw_extend_crc;
  uint32_t boot_times;
};
...
void
env_load (void); //load env value from MCU inside flash. 
uint8_t sys_env_save(struct sSystemEnv* sys); //save env value to MCU inside flash.

uint8_t user_env_save(EnvTable *env); //save env value to MCU inside flash.
#endif
