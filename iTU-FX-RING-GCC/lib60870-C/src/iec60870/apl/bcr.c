...
BinaryCounterReading_create (BinaryCounterReading self, int32_t value,
			     int seqNumber,
			     bool hasCarry,
			     bool isAdjusted, bool isInvalid)
{
  if (self == NULL)
    self = (BinaryCounterReading) GLOBAL_MALLOC(
	sizeof(struct sBinaryCounterReading));
