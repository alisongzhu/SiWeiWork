...
/* Private function prototypes -----------------------------------------------*/
static void
vTaskDTU_TCP_RX (void const * argument);
static void
vTaskDTU_COM_RX (void const * argument);
...
vTaskDTU_TCP_RX (void const * argument)
{
  ...
  MX_DMA_Init ();
  BSP_USART6_UART_Init (com->baudrate, com->ucDataBits,
			(eMBParity) com->eParity);
	      ...
	      if (connected == 1)
		{
		  ...
		  while (netconn_recv (dtu_client, &buf) == ERR_OK)
		      ...
		      do
			{
			  ...
			  HAL_GPIO_WritePin (RS485_USART6_T_R_GPIO_Port,
					     RS485_USART6_T_R_Pin,
					     GPIO_PIN_SET);
...
//	sem = xSemaphoreCreateBinary();
  status = xTaskCreate ((TaskFunction_t) vTaskDTU_TCP_RX, "TCP_RX", 200, NULL,
			3, &DTU_TCP_RX_Handle);
  xTaskCreate ((TaskFunction_t) vTaskDTU_COM_RX, "COM_RX", 200, NULL, 3,
	       &DTU_COM_RX_Handle);
  if (status != pdPASS)
    {
      //TODO: error
