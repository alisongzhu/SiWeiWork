...
//			env.LogicalDiscreteIndividual[i].mode = IEC104_OI_MODE_SP;
      ...
      env.LogicalDiscreteIndividual[i].CombinationalMode =
      COMBINATIONAL_MODE_SP;
      env.LogicalDiscreteIndividual[i].combination.sp = i;
...
env_load (void)
{
  ...
  FLASH_If_Read (SYSTEM_ENV_ADDRESS, (uint32_t *) &SystemEnv,
		 sizeof(struct sSystemEnv) / 4);
 ...
 */
uint8_t sys_env_save(struct sSystemEnv* sys)
{
  uint8_t result=0;
 ...
 * @return 0 success,other failure
 */
uint8_t user_env_save(EnvTable *env)
{
  uint8_t result=0;

  FLASH_If_Init();

  uint32_t user_env_address = USER_ENV_ADDRESS;

  if(FLASHIF_OK == FLASH_If_Erase(USER_ENV_ADDRESS,USER_ENV_SECTORS))
  {
    if(FLASHIF_OK==FLASH_If_Write(&user_env_address,(uint32_t*)env,sizeof(EnvTable)/4))
    {
      HAL_FLASH_Lock();
    }
    else{
