...
#define RXQ_TIMEOUT       5
#define TXQ_TIMEOUT       5
/* Private types -------------------------------------------------------------*/
//enum {
//	CAN_FRAME_CODE_INIT;
//}
...
static void
vTaskCanNetworkRx (void const * argument);
static void
vTaskCanNetworkTx (void const * argument);
static void
prvvParseMessage (CanRxMsgTypeDef RxMsg);
//static void prvvCanMemAlloc(void);
static void
prvvParseInitFrame (CanRxMsgTypeDef RxMsg);
static void
prvvParseDataFrame (CanRxMsgTypeDef RxMsg, uint8_t GenEvt);
static void
prvvTransmitRemoteFrame (uint8_t address, uint8_t type, uint8_t order,
			 uint8_t command, uint8_t reserve);
static void
Set_FilterConfig (uint8_t FilterNumber, uint32_t FilterID, uint32_t Mask);
static void
handleTimeOut (void);
...
//    Error_Handler();
  ...
  while (1)
    {
      xStatus = xQueueReceive(CanNetwork->RxQueue, &RxMsg, RXQ_TIMEOUT);
      if (xStatus == pdPASS)
	{
...
{
  BaseType_t xStatus;
  prvvTransmitRemoteFrame (0x1f, 0x0f, 0xff, CAN_FRAME_COMMAND_INIT, 0);
  while (1)
    {
      xStatus = xQueueReceive(CanNetwork->TxQueue, &CanNetwork->TxMessage,
			      TXQ_TIMEOUT);
      if (xStatus == pdPASS)
	{
...
//		printf("Wrong address!\r\n");
  ...
  struct sCanDevice *dev = (struct sCanDevice*) (CanNetwork->CanDevList)[address
      - 1];
  ...
  switch (command)
    ...
    case CAN_FRAME_COMMAND_READ:
      if (NULL == dev)
	{
	  prvvTransmitRemoteFrame (address, 0x0f, 0xff, CAN_FRAME_COMMAND_INIT,
				   0);
	}
      else
	{
...
prvvParseInitFrame (CanRxMsgTypeDef RxMsg)
{
  ...
  if (NULL == dev)
      ...
      if (dev->in32floatCount)
	{
      ...
      prvvTransmitRemoteFrame (address, 0x0f, 0xff, CAN_FRAME_COMMAND_BROADCAST,
			       0);
    }
}
...
prvvParseDataFrame (CanRxMsgTypeDef RxMsg, uint8_t GenEvt)
{
  ...
  struct sCanDevice * dev =
      (struct sCanDevice*) (CanNetwork->CanDevList)[address - 1];
  switch (type)
    {
...
Set_FilterConfig (uint8_t FilterNumber, uint32_t FilterID, uint32_t Mask)
{
  ...
  ;
  sFilterConfig.FilterIdLow = (((uint32_t) FilterID << 3) | CAN_ID_EXT
      | CAN_RTR_DATA) & 0xFFFF;
  ;
...
/* Exported functions --------------------------------------------------------*/
void
CanNetwork_init (void)
{
  if (env.BasicEnv.CanDevCount > 0)
    {
      CanNetwork = calloc (1, sizeof(struct sCanNetwork));

      CanNetwork->CanDevList = calloc (env.BasicEnv.CanDevCount,
				       sizeof(struct sCanDevice*));
      ...
      CanNetwork->RxQueue = xQueueCreate(CAN_NETWORK_RX_QLEN,
					 sizeof(CanRxMsgTypeDef));
//      vQueueAddToRegistry( CanNetwork->RxQueue, "CanNetworkRxQ" );

      CanNetwork->TxQueue = xQueueCreate(CAN_NETWORK_TX_QLEN,
					 sizeof(CanTxMsgTypeDef));
//      vQueueAddToRegistry( CanNetwork->TxQueue, "CanNetworkTxQ" );

      CanNetwork->In32FloatEventQueue = xQueueCreate(
	  CAN_NETWORK_MVS_EVENT_QLEN, sizeof(struct sIn32FloatEvent));
//      vQueueAddToRegistry( CanNetwork->MeasuredValueShortEventQueue, "MeasuredValueShortEventQ" );

      CanNetwork->In8EventQueue = xQueueCreate(CAN_NETWORK_SP_EVENT_QLEN,
					       sizeof(struct sIn8Event));
...
HAL_CAN_RxCpltCallback (CAN_HandleTypeDef* hcan)
{
  ...
  xQueueSendFromISR(CanNetwork->RxQueue, (void * )&CanNetwork->RxMessage,
