...
typedef enum
{
  TYPE_IOA_UNKNOWN, TYPE_IOA_INTERNAL, TYPE_IOA_EXTERNAL
} IOA_TYPE;
...
static bool
clockSyncHandler (void* parameter, MasterConnection connection, ASDU asdu,
		  CP56Time2a newTime);
static void
prvvProcessExternalIn32FloatData (MasterConnection connection);
static void
prvvProcessExternalIn8Data (MasterConnection connection);
static void
prvvProcessInternalData (MasterConnection connection);
static bool
interrogationHandler (void* parameter, MasterConnection connection, ASDU asdu,
		      uint8_t qoi);
static bool
...
isValidIOA (int ioa);
static void
processSingleCommand (ASDU asdu, InformationObject io);
static void
processDoubleCommand (ASDU asdu, InformationObject io);
static void
ExecuteOutput (int LogicalDiscreteNo, bool onoff);
//static bool Asdu_isValidCA(ASDU self);
static bool
IsCommandExecutable (ASDU asdu);
static void
prvvProcessExternalIn32FloatEvent (void);
static void
prvvProcessExternalIn8Event (void);
static void
prvvProcessInternalEvent (void);

static void
handleSelectSwitchTimeout (void);

static void
vTaskIEC104 (void const * argument);
/* Private functions ---------------------------------------------------------*/
static bool
clockSyncHandler (void* parameter, MasterConnection connection, ASDU asdu,
		  CP56Time2a newTime)
...
prvvProcessExternalIn32FloatData (MasterConnection connection)
{
  ...
  for (uint8_t i = 0; i < env.BasicEnv.CanDevCount; i++)
	...
	{
	  ASDU Asdu = ASDU_create (connectionParameters, M_ME_NC_1, true,
				   INTERROGATED_BY_STATION, 0,
				   env.IEC104Env.address, false, false);
  ...
  for (uint8_t i = 0; i < env.BasicEnv.CanDevCount; i++)
      ...
      if ((dev != NULL) && (dev->data->in8 != NULL))
	{
	  if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_SP)
	    {
	      ASDU Asdu = ASDU_create (connectionParameters, M_SP_NA_1, true,
				       INTERROGATED_BY_STATION, 0,
				       env.IEC104Env.address, false, false);
	      InformationObject io = NULL;
	      for (uint8_t j = 0; j < dev->in8Count; j++)
		{
		  io = (InformationObject) SinglePointInformation_create (
		      ...
		      EXTERNAL_DISCRETE_START_ADDR
			  + i * EXTERNAL_DEV_DISCRETE_CNT + j,
		      dev->data->in8[j] & 1,
		      IEC60870_QUALITY_GOOD);
		  ASDU_addInformationObject (Asdu, io);
		}
	  ...
	  if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_DP)
	    {
	      ASDU Asdu = ASDU_create (connectionParameters, M_DP_NA_1, true,
				       INTERROGATED_BY_STATION, 0,
				       env.IEC104Env.address, false, false);
	      InformationObject io = NULL;
	      for (uint8_t j = 0; j < dev->in8Count; j++)
...
prvvProcessInternalData (MasterConnection connection)
{
  ...
  if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_SP)
    {
      Asdu = ASDU_create (connectionParameters, M_SP_NA_1, true,
			  INTERROGATED_BY_STATION, 0, env.IEC104Env.address,
			  false, false);
    }
  else if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_DP)
    {
      Asdu = ASDU_create (connectionParameters, M_DP_NA_1, true,
			  INTERROGATED_BY_STATION, 0, env.IEC104Env.address,
			  false, false);
    }
...
isValidIOA (int ioa)
{
  if ((ioa < INTERNAL_COIL_START_ADDR)
      || ioa > (INTERNAL_COIL_START_ADDR + env.BasicEnv.InternalCoilCNT))
    return false;
  return true;
}
...
ExecuteOutput (int LogicalDiscreteNo, bool onoff)
{
  struct sLogicalCoilIndividual *obj =
      &env.LogicalCoilIndividual[LogicalDiscreteNo];
  if (onoff)
    {
...
processSingleCommand (ASDU asdu, InformationObject io)
{
  ...
  if (SingleCommand_isSelect (sc))
	...
	{
	  env.LogicalCoilIndividual[ioa - INTERNAL_COIL_START_ADDR].Selected =
	      1;
      ...
      else
	{
	  if ((0
	      == env.LogicalCoilIndividual[ioa - INTERNAL_COIL_START_ADDR].Selected)
	      && (GPIO_PIN_SET
		  == HAL_GPIO_ReadPin (SELECT_FB_GPIO_Port, SELECT_FB_Pin)))
	    {
	      SELECT_ON();
	      env.LogicalCoilIndividual[ioa - INTERNAL_COIL_START_ADDR].Selected =
		  1;
      ...
      if (env.BasicEnv.CoilOutputMode == COIL_MODE_SBO)
	{
	  if (0
	    ...
	    {
	      ExecuteOutput (ioa - INTERNAL_COIL_START_ADDR,
      ...
      if (env.BasicEnv.CoilOutputMode == COIL_MODE_SBO)
	{
	  if (0
	  ...
	  else
	    {
	      ...
	      if ((val == IEC60870_DOUBLE_POINT_INTERMEDIATE)
		  || (val == IEC60870_DOUBLE_POINT_INDETERMINATE))
		{
		  ASDU_setCOT (asdu, ACTIVATION_TERMINATION);
		}
	      else
...
asduHandler (void* parameter, MasterConnection connection, ASDU asdu)
{
  ...
  if (ASDU_getTypeID (asdu) == C_SC_TA_1)
      ...
      if (!IsCommandExecutable (asdu))
	{
      ...
      if (TickInMs
	  > (CP56Time2a_toMsTimestamp (SingleCmdTime)
	      + COMMAND_WITHCP56TIMEA_TIMEOUT))
	{
  ...
  if (ASDU_getTypeID (asdu) == C_DC_TA_1)
      ...
      if (!IsCommandExecutable (asdu))
	{
      ...
      if (TickInMs
	  > (CP56Time2a_toMsTimestamp (DoubleCmdTime)
	      + COMMAND_WITHCP56TIMEA_TIMEOUT))
	{
	  DEBUG_PRINT("wrong command time stamp!\n");
	  ASDU_setCOT (asdu, ACTIVATION_TERMINATION);
...
prvvProcessExternalIn32FloatEvent (void)
{
  UBaseType_t uxNumberOfItems = uxQueueMessagesWaiting (
      CanNetwork->In32FloatEventQueue);
  if (uxNumberOfItems)
    {
...
//            }
      ...
      ASDU Asdu = ASDU_create (connectionParameters, M_ME_NC_1, false,
			       SPONTANEOUS, 0, env.IEC104Env.address, false,
			       false);
      InformationObject io = NULL;
      ...
      for (uint8_t i = 0; i < uxNumberOfItems; i++) //todo: max asdu lenth == 256.
	{
	  ...
	  io = (InformationObject) MeasuredValueShort_create (
	      (MeasuredValueShort) io,
	      EXTERNAL_INPUT_START_ADDR
		  + (item.address - 1) * EXTERNAL_DEV_INPUT_CNT
		  + (item.order - 1),
	      item.val,
	      IEC60870_QUALITY_GOOD);
	  ASDU_addInformationObject (Asdu, io);
...
{
  UBaseType_t uxNumberOfItems = uxQueueMessagesWaiting (
      CanNetwork->In8EventQueue);
  if (uxNumberOfItems)
    {
      struct sIn8Event item;
      if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_SP)
	{
	  ASDU Asdu = ASDU_create (connectionParameters, M_SP_NA_1, false,
				   SPONTANEOUS, 0, env.IEC104Env.address, false,
				   false);
	  InformationObject io = NULL;
	  for (uint8_t i = 0; i < uxNumberOfItems; i++)
	    {
		  ...
		  (SinglePointInformation) io,
		  EXTERNAL_DISCRETE_START_ADDR
		      + (item.address - 1) * EXTERNAL_DEV_DISCRETE_CNT
		      + item.order,
		  item.val & 1,
		  IEC60870_QUALITY_GOOD);
	      ASDU_addInformationObject (Asdu, io);
      ...
      if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_DP)
	{
	  ASDU Asdu = ASDU_create (connectionParameters, M_DP_NA_1, false,
				   SPONTANEOUS, 0, env.IEC104Env.address, false,
				   false);
	  InformationObject io = NULL;
	  for (uint8_t i = 0; i < uxNumberOfItems; i++)
      ...
      while (uxNumberOfItems--)
	{
	  ...
	  if (T104Slave_getOpenConnections (slave))
	      ...
	      if (1 == env.LogicalDiscreteIndividual[item.flag].GenEvent)
		{
		  if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_DP)
		    {
		      ASDU Asdu = ASDU_create (connectionParameters, M_DP_NA_1,
					       false, SPONTANEOUS, 0,
					       env.IEC104Env.address, false,
					       false);
		      InformationObject io =
			  (InformationObject) DoublePointInformation_create (
			      NULL,
		  ...
		  else if (env.LogicalDiscreteGlobal.ObjectMode
		      == IEC104_OI_MODE_SP)
		    {
		      ASDU Asdu = ASDU_create (connectionParameters, M_SP_NA_1,
					       false, SPONTANEOUS, 0,
					       env.IEC104Env.address, false,
					       false);
		      InformationObject io =
	  ...
	  if (1 == env.LogicalDiscreteIndividual[item.flag].GenSOE)
	    {
	      ...
	      if (env.LogicalDiscreteGlobal.ObjectMode == IEC104_OI_MODE_SP)
		{
		  ASDU Asdu = ASDU_create (connectionParameters, M_SP_TB_1,
					   false, SPONTANEOUS, 0,
					   env.IEC104Env.address, false, false);
		  InformationObject io =
		      (InformationObject) SinglePointWithCP56Time2a_create (
			  NULL,
			  INTERNAL_DISCRETE_START_ADDR + item.flag,
			  item.val & 1,
			  IEC60870_QUALITY_GOOD,
		...
		{
		  ASDU Asdu = ASDU_create (connectionParameters, M_DP_TB_1,
					   false, SPONTANEOUS, 0,
					   env.IEC104Env.address, false, false);
		  InformationObject io =
		      (InformationObject) DoublePointWithCP56Time2a_create (
			  NULL,
			  INTERNAL_DISCRETE_START_ADDR + item.flag,
			  (DoublePointValue) item.val,
			  IEC60870_QUALITY_GOOD,
  ...
  if ( COIL_MODE_SBO == env.BasicEnv.CoilOutputMode)
    {
      if ((GPIO_PIN_RESET
	  == HAL_GPIO_ReadPin (SELECT_FB_GPIO_Port, SELECT_FB_Pin))
	  && ((TimeSelectOn + env.IEC104Env.SBO_Deadband * 1000)
	      < HAL_GetTick ()))
	{
	  HAL_GPIO_WritePin (SELECT_DO_GPIO_Port, SELECT_DO_Pin,
			     GPIO_PIN_RESET);
	  uint8_t i;
	  for (i = 0; i < env.BasicEnv.InternalCoilCNT; i++)
	    {
