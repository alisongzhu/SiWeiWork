  ...
  if (state == IDLE)
      ...
      if (allTransmitReady (bridge, givenPort))
	{
	  ...
	  if (!port->sendRSTP && port->newInfo
	      && cistDesignatedPort (bridge, givenPort)
	      && (port->txCount < bridge->TxHoldCount)
	      && (port->helloWhen != 0))
	    return TRANSMIT_CONFIG;

	  if (!port->sendRSTP && port->newInfo
	      && cistRootPort (bridge, givenPort)
	      && (port->txCount < bridge->TxHoldCount)
	      && (port->helloWhen != 0))
	    return TRANSMIT_TCN;
	  ...
	  if (port->sendRSTP
	      && (port->newInfo
		  || (port->newInfoMsti && !mstiMasterPort (bridge, givenPort)))
	      && (port->txCount < bridge->TxHoldCount)
	      && (port->helloWhen != 0))
	    return TRANSMIT_RSTP;
	}
				    ...
				    unsigned int timestamp)
{
  ...
  if (state == TRANSMIT_INIT)
  ...
  else if (state == TRANSMIT_PERIODIC)
    {
	  ...
	  || (cistDesignatedPort (bridge, givenPort)
	      || (cistRootPort (bridge, givenPort)
		  && (port->trees[CIST_INDEX]->tcWhile != 0)));
      port->newInfoMsti = port->newInfoMsti
	  || mstiDesignatedOrTCpropagatingRootPort (bridge, givenPort);
    }
