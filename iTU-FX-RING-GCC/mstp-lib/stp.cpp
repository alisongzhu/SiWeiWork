...
static void
RunStateMachines (STP_BRIDGE* bridge, unsigned int timestamp);
static unsigned int
GetInstanceCountForAllStateMachines (STP_BRIDGE* bridge);
static void
RecomputePrioritiesAndPortRoles (STP_BRIDGE* bridge, unsigned int treeIndex,
				 unsigned int timestamp);
static void
ComputeMstConfigDigest (STP_BRIDGE* bridge);

// ============================================================================

STP_BRIDGE*
STP_CreateBridge (unsigned int portCount, unsigned int mstiCount,
		  unsigned int maxVlanNumber, const STP_CALLBACKS* callbacks,
		  ...
		  unsigned int debugLogBufferSize)
{
  ...
  STP_BRIDGE* bridge = (STP_BRIDGE*) callbacks->allocAndZeroMemory (
      sizeof(STP_BRIDGE));
  assert(bridge != NULL);
  ...
  bridge->logBuffer = (char*) callbacks->allocAndZeroMemory (
      debugLogBufferSize);
  ...
  unsigned int stateMachineInstanceCount = GetInstanceCountForAllStateMachines (
      bridge);
  bridge->states = (SM_STATE*) callbacks->allocAndZeroMemory (
      stateMachineInstanceCount * sizeof(SM_STATE));
  assert(bridge->states != NULL);

  // ------------------------------------------------------------------------

  bridge->trees = (BRIDGE_TREE**) callbacks->allocAndZeroMemory (
      (1 + bridge->mstiCount) * sizeof(BRIDGE_TREE*));
  assert(bridge->trees != NULL);

  bridge->ports = (PORT**) callbacks->allocAndZeroMemory (
      portCount * sizeof(PORT*));
  assert(bridge->ports != NULL);

  // per-bridge CIST vars
  bridge->trees[CIST_INDEX] = (BRIDGE_TREE*) callbacks->allocAndZeroMemory (
      sizeof(BRIDGE_TREE));
  assert(bridge->trees [CIST_INDEX] != NULL);
  bridge->trees[CIST_INDEX]->SetBridgeIdentifier (0x8000, CIST_INDEX,
						  bridgeAddress);
  // 13.24.3 in 802.1Q-2011
  bridge->trees[CIST_INDEX]->BridgeTimes.HelloTime =
      STP_BRIDGE::BridgeHelloTime;
  bridge->trees[CIST_INDEX]->BridgeTimes.remainingHops = bridge->MaxHops;
  bridge->trees[CIST_INDEX]->BridgeTimes.ForwardDelay =
      bridge->BridgeForwardDelay;
  ...
  for (unsigned int treeIndex = 1; treeIndex < (1 + bridge->mstiCount);
      treeIndex++)
    {
      bridge->trees[treeIndex] = (BRIDGE_TREE*) callbacks->allocAndZeroMemory (
	  sizeof(BRIDGE_TREE));
      assert(bridge->trees [treeIndex] != NULL);
      bridge->trees[treeIndex]->SetBridgeIdentifier (0x8000, treeIndex,
						     bridgeAddress);
  ...
  for (unsigned int portIndex = 0; portIndex < bridge->portCount; portIndex++)
    {
      bridge->ports[portIndex] = (PORT*) callbacks->allocAndZeroMemory (
	  sizeof(PORT));
      assert(bridge->ports [portIndex] != NULL);

      PORT* port = bridge->ports[portIndex];

      port->trees = (PORT_TREE**) callbacks->allocAndZeroMemory (
	  (1 + bridge->mstiCount) * sizeof(PORT_TREE*));
      assert(port->trees != NULL);

      // per-port CIST and MSTI vars
      for (unsigned int treeIndex = 0; treeIndex < (1 + bridge->mstiCount);
	  treeIndex++)
	{
	  port->trees[treeIndex] = (PORT_TREE*) callbacks->allocAndZeroMemory (
	      sizeof(PORT_TREE));
	  assert(port->trees [treeIndex] != NULL);
	  port->trees[treeIndex]->portId.Set (0x80,
					      (unsigned short) portIndex + 1);
	  port->trees[treeIndex]->portTimes.HelloTime =
	      STP_BRIDGE::BridgeHelloTime;
  ...
  STP_GetDefaultMstConfigName (bridgeAddress,
			       bridge->MstConfigId.ConfigurationName);

  bridge->mstConfigTable = (INV_UINT2*) callbacks->allocAndZeroMemory (
      (1 + maxVlanNumber) * 2);
...
void
STP_DestroyBridge (STP_BRIDGE* bridge)
{
  ...
  for (unsigned int portIndex = 0; portIndex < bridge->portCount; portIndex++)
    {
      for (unsigned int treeIndex = 0; treeIndex < (1 + bridge->mstiCount);
	  treeIndex++)
	bridge->callbacks.freeMemory (
	    bridge->ports[portIndex]->trees[treeIndex]);
...
void
STP_StartBridge (STP_BRIDGE* bridge, unsigned int timestamp)
{
...
void
STP_StopBridge (STP_BRIDGE* bridge, unsigned int timestamp)
{
...
void
STP_SetBridgeAddress (STP_BRIDGE* bridge, const unsigned char* address,
		      unsigned int timestamp)
{
  LOG(bridge, -1, -1, "{T}: Setting bridge MAC address to {BA}...", timestamp,
      address);

  const unsigned char* currentAddress =
      bridge->trees[CIST_INDEX]->GetBridgeIdentifier ().GetAddress ().bytes;
...
const struct STP_BRIDGE_ADDRESS*
STP_GetBridgeAddress (const struct STP_BRIDGE* bridge)
{
...
static unsigned int
GetDefaultPortPathCost (unsigned int speedMegabitsPerSecond)
{
...
void
STP_OnPortEnabled (STP_BRIDGE* bridge, unsigned int portIndex,
		   unsigned int speedMegabitsPerSecond,
		   unsigned int detectedPointToPointMAC, unsigned int timestamp)
{
  ...
  if (port->AdminExternalPortPathCost != 0)
    ...
    port->ExternalPortPathCost = GetDefaultPortPathCost (
	speedMegabitsPerSecond);
...
void
STP_OnPortDisabled (STP_BRIDGE* bridge, unsigned int portIndex,
		    unsigned int timestamp)
{
...
void
STP_OnOneSecondTick (STP_BRIDGE* bridge, unsigned int timestamp)
{
  if (bridge->started)
    {
	...
	bridge->tcIgnore--;
      ...
      for (unsigned int givenPort = 0; givenPort < bridge->portCount;
	  givenPort++)
...
void
STP_OnBpduReceived (STP_BRIDGE* bridge, unsigned int portIndex,
		    const unsigned char* bpdu, unsigned int bpduSize,
		    unsigned int timestamp)
{
  if (bridge->started)
    {
      if (bridge->ports[portIndex]->portEnabled == false)
	{
	  LOG(bridge,
	      -1,
	      -1,
	      "{T}: WARNING: BPDU received on disabled port {D}. The STP library is discarding it.\r\n",
	      timestamp, 1 + portIndex);
	}
      else
	{
	  ...
	  enum VALIDATED_BPDU_TYPE type = STP_GetValidatedBpduType (bpdu,
								    bpduSize);
	  ...
	  switch (type)
	    ...
	    default:
	      LOG(bridge, portIndex, -1,
		  "Invalid BPDU received. Discarding it.\r\n");
...
unsigned int
STP_IsBridgeStarted (const STP_BRIDGE* bridge)
{
...
void
STP_EnableLogging (STP_BRIDGE* bridge, unsigned int enable)
{
...
unsigned int
STP_IsLoggingEnabled (const STP_BRIDGE* bridge)
{
...
static bool
RunStateMachineInstance (STP_BRIDGE* bridge, const SM_INFO* smInfo,
			 int givenPort, int givenTree, SM_STATE* statePtr,
			 unsigned int timestamp)
{
  volatile bool changed = false;

  rep: SM_STATE newState = smInfo->checkConditions (bridge, givenPort,
						    givenTree, *statePtr);
  if (newState != 0)
    {
      if (givenPort == -1)
...
static bool
RunStateMachineInstances (STP_BRIDGE* bridge, const SM_INFO* smInfo,
			  SM_STATE** statePtr, unsigned int timestamp)
{
  volatile bool changed = false;
  ...
  switch (smInfo->instanceType)
    ...
    case SM_INFO::PER_BRIDGE_PER_TREE:
      for (unsigned int treeIndex = 0; treeIndex < bridge->treeCount ();
	  treeIndex++)
	{
	  changed |= RunStateMachineInstance (bridge, smInfo, -1, treeIndex,
					      *statePtr, timestamp);
	  (*statePtr)++;
	}
      break;
    ...
    case SM_INFO::PER_PORT:
      for (unsigned int portIndex = 0; portIndex < bridge->portCount;
	  portIndex++)
	{
	  changed |= RunStateMachineInstance (bridge, smInfo, portIndex, -1,
					      *statePtr, timestamp);
	  (*statePtr)++;
	}
      break;
	  ...
	  treeIndex++)
	{
	  for (unsigned int portIndex = 0; portIndex < bridge->portCount;
	      portIndex++)
	    {
	      changed |= RunStateMachineInstance (bridge, smInfo, portIndex,
						  treeIndex, *statePtr,
						  timestamp);
	      (*statePtr)++;
	    }
...
RunStateMachines (STP_BRIDGE* bridge, unsigned int timestamp)
{
  ...
  do
	...
	changed |= RunStateMachineInstances (bridge,
					     &bridge->smInterface->smInfo[i],
					     &statePtr, timestamp);
      ...
      if (changed == false)
	changed |= RunStateMachineInstances (
	    bridge, bridge->smInterface->transmitSmInfo, &statePtr, timestamp);
...
static void
RestartStateMachines (STP_BRIDGE* bridge, unsigned int timestamp)
{
  assert(bridge->states);
  memset (bridge->states, 0,
	  GetInstanceCountForAllStateMachines (bridge) * sizeof(SM_STATE));
  bridge->BEGIN = true;
  RunStateMachines (bridge, timestamp);
  bridge->BEGIN = false;
  RunStateMachines (bridge, timestamp);
...
static unsigned int
GetInstanceCountForStateMachine (const SM_INFO* smInfo, unsigned int portCount,
...
GetInstanceCountForAllStateMachines (STP_BRIDGE* bridge)
{
  ...
  for (unsigned int i = 0; i < bridge->smInterface->smInfoCount; i++)
    count += GetInstanceCountForStateMachine (&bridge->smInterface->smInfo[i],
					      bridge->portCount,
					      (1 + bridge->mstiCount));

  count += GetInstanceCountForStateMachine (bridge->smInterface->transmitSmInfo,
					    bridge->portCount,
					    (1 + bridge->mstiCount));
...
void
STP_SetPortAdminEdge (STP_BRIDGE* bridge, unsigned int portIndex,
		      unsigned int newAdminEdge, unsigned int timestamp)
{
  bridge->ports[portIndex]->AdminEdge = newAdminEdge;
}

unsigned int
STP_GetPortAdminEdge (const STP_BRIDGE* bridge, unsigned int portIndex)
{
...
void
STP_SetPortAutoEdge (STP_BRIDGE* bridge, unsigned int portIndex,
		     unsigned int newAutoEdge, unsigned int timestamp)
{
  bridge->ports[portIndex]->AutoEdge = newAutoEdge;
}

unsigned int
STP_GetPortAutoEdge (const STP_BRIDGE* bridge, unsigned int portIndex)
{
...
void
STP_SetPortAdminPointToPointMAC (STP_BRIDGE* bridge, unsigned int portIndex,
				 STP_ADMIN_P2P adminPointToPointMAC,
				 unsigned int timestamp)
{
  const char* p2pString = STP_GetAdminP2PString (adminPointToPointMAC);
  LOG(bridge, portIndex, -1,
      "{T}: Setting adminPointToPointMAC = {S} on port {D}...\r\n", timestamp,
...
STP_ADMIN_P2P
STP_GetPortAdminPointToPointMAC (const STP_BRIDGE* bridge,
...
void
STP_SetBridgePriority (STP_BRIDGE* bridge, unsigned int treeIndex,
		       unsigned short bridgePriority, unsigned int timestamp)
{
  ...
  LOG(bridge, -1, -1,
      "{T}: Setting bridge priority: tree {TN} prio = {D}...\r\n", timestamp,
...
unsigned short
STP_GetBridgePriority (const STP_BRIDGE* bridge, unsigned int treeIndex)
{
  assert(treeIndex <= bridge->mstiCount);
...
void
STP_SetPortPriority (STP_BRIDGE* bridge, unsigned int portIndex,
		     unsigned int treeIndex, unsigned char portPriority,
		     unsigned int timestamp)
{
  // See table 13-3 on page 348 of 802.1Q-2011
  ...
  LOG(bridge, -1, -1,
      "{T}: Setting port priority: port {D} tree {TN} prio = {D}...\r\n",
      timestamp, 1 + portIndex, treeIndex, portPriority);
...
unsigned char
STP_GetPortPriority (const STP_BRIDGE* bridge, unsigned int portIndex,
		     unsigned int treeIndex)
{
  ...
  unsigned char priority =
      bridge->ports[portIndex]->trees[treeIndex]->portId.GetPriority ();
  return priority;
}

unsigned short
STP_GetPortIdentifier (const STP_BRIDGE* bridge, unsigned int portIndex,
		       unsigned int treeIndex)
{
  assert(portIndex < bridge->portCount);
  assert(treeIndex <= bridge->mstiCount);
  ...
  unsigned short id =
      bridge->ports[portIndex]->trees[treeIndex]->portId.GetPortIdentifier ();
  return id;
}
...
void
STP_GetDefaultMstConfigName (const unsigned char bridgeAddress[6],
			     char nameOut[18])
{
...
void
STP_SetMstConfigName (STP_BRIDGE* bridge, const char* name,
		      unsigned int timestamp)
{
  ...
  if (bridge->started)
      ...
      else
	{
	  ...
	  LOG(bridge,
	      -1,
	      -1,
	      "(This has no effect right now as the bridge isn't configured for MSTP.\r\n");
	  STP_Unindent (bridge);
	}
    }
...
void
STP_SetMstConfigRevisionLevel (STP_BRIDGE* bridge, unsigned short revisionLevel,
			       unsigned int timestamp)
{
...
ComputeMstConfigDigest (STP_BRIDGE* bridge)
{
  ...
  HMAC_MD5_Update (&context, bridge->mstConfigTable,
		   2 * (1 + bridge->maxVlanNumber));
...
void
STP_SetMstConfigTable (struct STP_BRIDGE* bridge,
		       const STP_CONFIG_TABLE_ENTRY* entries,
		       unsigned int entryCount, unsigned int timestamp)
{
  ...
  if (memcmp (bridge->mstConfigTable, entries, entryCount * 2) == 0)
	...
	assert(entries[4095].treeIndex == 0);
	  ...
	  bridge->MstConfigId.ConfigurationDigest[0],
	  bridge->MstConfigId.ConfigurationDigest[1],
	  bridge->MstConfigId.ConfigurationDigest[14],
	  bridge->MstConfigId.ConfigurationDigest[15]);
...
const STP_CONFIG_TABLE_ENTRY*
STP_GetMstConfigTable (STP_BRIDGE* bridge, unsigned int* entryCountOut)
{
...
unsigned int
STP_GetPortCount (const STP_BRIDGE* bridge)
{
  return bridge->portCount;
}

unsigned int
STP_GetMstiCount (const STP_BRIDGE* bridge)
{
  return bridge->mstiCount;
}

enum STP_VERSION
STP_GetStpVersion (const STP_BRIDGE* bridge)
{
  return bridge->ForceProtocolVersion;
}

void
STP_SetStpVersion (STP_BRIDGE* bridge, enum STP_VERSION version,
		   unsigned int timestamp)
{
  LOG(bridge, -1, -1, "{T}: Switching to {S}... ", timestamp,
...
unsigned int
STP_GetPortEnabled (const STP_BRIDGE* bridge, unsigned int portIndex)
{
  return bridge->ports[portIndex]->portEnabled;
}

STP_PORT_ROLE
STP_GetPortRole (const STP_BRIDGE* bridge, unsigned int portIndex,
		 unsigned int treeIndex)
{
...
unsigned int
STP_GetPortLearning (const STP_BRIDGE* bridge, unsigned int portIndex,
		     unsigned int treeIndex)
{
  assert(bridge->started);
...
unsigned int
STP_GetPortForwarding (const STP_BRIDGE* bridge, unsigned int portIndex,
		       unsigned int treeIndex)
{
  assert(bridge->started);
...
unsigned int
STP_GetPortOperEdge (const STP_BRIDGE* bridge, unsigned int portIndex)
{
  assert(bridge->started);
...
unsigned int
STP_GetPortOperPointToPointMAC (const STP_BRIDGE* bridge,
				unsigned int portIndex)
{
...
unsigned int
STP_GetMaxVlanNumber (const STP_BRIDGE* bridge)
{
  return bridge->maxVlanNumber;
}

unsigned int
STP_GetTreeIndexFromVlanNumber (const STP_BRIDGE* bridge,
...
const struct STP_MST_CONFIG_ID*
STP_GetMstConfigId (const struct STP_BRIDGE* bridge)
{
...
const char*
STP_GetPortRoleString (STP_PORT_ROLE portRole)
{
  switch (portRole)
    {
...
const char*
STP_GetVersionString (enum STP_VERSION version)
{
  switch (version)
    {
    case STP_VERSION_LEGACY_STP:
      return LegacySTPString;
    case STP_VERSION_RSTP:
      return RSTPString;
    case STP_VERSION_MSTP:
      return MSTPString;
...
enum STP_VERSION
STP_GetVersionFromString (const char* str)
{
...
const char*
STP_GetAdminP2PString (enum STP_ADMIN_P2P adminP2P)
{
  switch (adminP2P)
    {
...
void
STP_GetRootPriorityVector (const STP_BRIDGE* bridge, unsigned int treeIndex,
			   unsigned char priorityVectorOut[36])
{
  assert(bridge->started);
  const unsigned char* rootPriority =
      (const unsigned char*) &bridge->trees[treeIndex]->rootPriority;
  const unsigned char* rootPortId =
      (const unsigned char*) &bridge->trees[treeIndex]->rootPortId;
...
void
STP_GetRootTimes (const STP_BRIDGE* bridge, unsigned int treeIndex,
...
unsigned int
STP_IsCistRoot (const STP_BRIDGE* bridge)
{
  assert(bridge->started);
...
unsigned int
STP_IsRegionalRoot (const STP_BRIDGE* bridge, unsigned int treeIndex)
{
  assert(bridge->started);
...
void
STP_SetApplicationContext (STP_BRIDGE* bridge, void* applicationContext)
{
  bridge->applicationContext = applicationContext;
}

void*
STP_GetApplicationContext (const STP_BRIDGE* bridge)
{
...
void
STP_MST_CONFIG_ID::Dump (STP_BRIDGE* bridge, int port, int tree) const
{
  char namesz[33];
  memcpy (namesz, ConfigurationName, 32);
  namesz[32] = 0;
  LOG(bridge, port, tree,
      "Name=\"{S}\", Rev={D}, Digest={X2}{X2}..{X2}{X2}\r\n", namesz,
      (RevisionLevelHigh << 8) | RevisionLevelLow, ConfigurationDigest[0],
      ConfigurationDigest[1], ConfigurationDigest[14], ConfigurationDigest[15]);
}

// ============================================================================

bool
STP_MST_CONFIG_ID::operator== (const STP_MST_CONFIG_ID& rhs) const
{
...
void
STP_SetAdminPortPathCost (struct STP_BRIDGE* bridge, unsigned int portIndex,
			  unsigned int adminPathCost,
...
unsigned int
STP_GetAdminPortPathCost (const struct STP_BRIDGE* bridge,
			  unsigned int portIndex)
{
  assert(false); // not implemented
  return 0;
...
unsigned int
STP_GetPortPathCost (const struct STP_BRIDGE* bridge, unsigned int portIndex)
{
  assert(false); // not implemented
  return 0;
}
