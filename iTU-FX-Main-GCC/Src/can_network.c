...
static void vTaskCanNetworkTx(void const * argument)
{
	...
	while(1)
	{
		xStatus = xQueueReceive(CanNetwork->TxQueue,&CanNetwork->TxMessage,TXQ_TIMEOUT);
		if(xStatus == pdPASS)
		{
...
//		printf("Wrong address!\r\n");
	...
	switch(command)
	{
		...
		case CAN_FRAME_COMMAND_READ:
			if(NULL == dev)
			{
				prvvTransmitRemoteFrame(address,0x0f,0xff,CAN_FRAME_COMMAND_INIT,0);
			}
			else
			{
...
static void prvvParseInitFrame(CanRxMsgTypeDef RxMsg)
{
	...
	if(NULL == dev)
	{
		...
		*p = dev;
		...
		prvvTransmitRemoteFrame(address,0x0f,0xff,CAN_FRAME_COMMAND_BROADCAST,0);
	}
}
