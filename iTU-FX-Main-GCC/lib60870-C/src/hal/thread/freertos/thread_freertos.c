...
Thread_start(Thread thread)
{
  if(thread->autodestroy == true ){
    osThreadDef(thread_autodestroy,(os_pthread)destroyAutomaticThread,IEC104_THREAD_PRIORITY,0,IEC104_AUTOMATIC_THREAD_STACK_SIZE);
    thread->handle = osThreadCreate(osThread(thread_autodestroy),thread);
  }
  else {
    osThreadDef(iec104_Svr,(os_pthread)thread->pthread,IEC104_THREAD_PRIORITY,0,IEC104_SERVER_THREAD_STACK_SIZE);
