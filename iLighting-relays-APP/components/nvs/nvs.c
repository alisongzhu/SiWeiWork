...
static int _nvs_flash_al_wrt(struct nvs_fs *fs, uint32_t addr, const void *data,
			     size_t len)
{
...
static int _nvs_flash_rd(struct nvs_fs *fs, uint32_t addr, void *data,
			 size_t len)
{
	int rc;
	off_t offset;
...
/* flash ate read */
static int _nvs_flash_ate_rd(struct nvs_fs *fs, uint32_t addr,
 ...
 */
static int _nvs_flash_block_cmp(struct nvs_fs *fs, uint32_t addr, const void *data,
				size_t len)
{
	const uint8_t *data8 = (const uint8_t *)data;
	int rc;
 ...
 * errcode if error
 */
static int _nvs_flash_cmp_const(struct nvs_fs *fs, uint32_t addr, uint8_t value,
				size_t len)
{
	int rc;
	size_t bytes_to_cmp, block_size;
 ...
 */
static int _nvs_flash_block_move(struct nvs_fs *fs, uint32_t addr, size_t len)
{
	int rc;
 ...
 */
static int _nvs_flash_erase_sector(struct nvs_fs *fs, uint32_t addr)
{
	int rc;
	off_t offset;
...
static int _nvs_flash_wrt_entry(struct nvs_fs *fs, uint16_t id, const void *data,
				size_t len)
{
	int rc;
	struct nvs_ate entry;
 ...
 */
static int _nvs_prev_ate(struct nvs_fs *fs, uint32_t *addr, struct nvs_ate *ate)
{
	int rc;
	size_t ate_size;
...
static void _nvs_sector_advance(struct nvs_fs *fs, uint32_t *addr)
{
	*addr += (1 << ADDR_SECT_SHIFT);
...
{
	int rc;
	uint8_t buf[sizeof(struct nvs_ate)];
	size_t ate_size;
...
static int _nvs_gc(struct nvs_fs *fs)
{
	...
	uint32_t gc_addr, wlk_addr, wlk_prev_addr, data_addr, sec_addr;
	size_t ate_size;
...
int nvs_reinit(struct nvs_fs *fs)
{
	...
	xSemaphoreTake(fs->nvs_lock, portMAX_DELAY);
...
int nvs_init(struct nvs_fs *fs, const struct device *dev)
{
	...
	fs->nvs_lock = xSemaphoreCreateMutex();
...
ssize_t nvs_write(struct nvs_fs *fs, uint16_t id, const void *data, size_t len)
{
	...
	uint32_t wlk_addr, rd_addr, freed_space;
	uint16_t sector_freespace;
