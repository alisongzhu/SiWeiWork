...
void flash_stm32_page_layout(struct device *dev,
			     ...
			     size_t *layout_size)
{
	static struct flash_pages_layout stm32f1_flash_layout = {
		.pages_count = 0,
		.pages_size = 0,
	};
	...
	if (stm32f1_flash_layout.pages_count == 0) {
		stm32f1_flash_layout.pages_count = (CONFIG_FLASH_SIZE * 1024) /
							FLASH_PAGE_SIZE;
		stm32f1_flash_layout.pages_size = FLASH_PAGE_SIZE;
	}

	*layout = &stm32f1_flash_layout;
	*layout_size = 1;
}
