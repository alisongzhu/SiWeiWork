...
typedef struct EnergySavedef EnergySavedef;
 const Rn8302_ConstParadef  Rn8302_Const[READ_LIST_NUM]={
	...
	{
		.Address=0x038,/*A正向无功电能*/
		.Size=3,
		.DataHandle=energy
	},/*28*/
	{
		.Address=0x039,/*/*B正向无功电能*/
		.Size=3,
		.DataHandle=energy
	},/*29*/
	{
		.Address=0x03a,/*C正向无功电能*/
		.Size=3,
		.DataHandle=energy
	},/*30*/
	{
...
{
  "set_ratio",
  "\r\nset_ratio:\r\n set ratio IRmsConst1-3 VRmsConst1-3 ;\r\n",
  prvWriteRatio,
  6
};
...
{
  "set_deadzone",
  "\r\nset_deadzone:\r\n set deadzone IdzConst1-3 VdzConst1-3 egg:set_deadzone 10 10 10 0.1 0.1 0.1;\r\n",
  prvWriteDeadZone,
  6
};
...
static float ProcVoltageData(int voltage,uint16_t addr)
{
	...
	Value = (float)(((float)voltage)/VOLTAGE_CONST*RN8302RatioPara.VRmsConst[i]);
...
static uint64_t prvSaveEnergy_Fram(uint32_t EnergyData,uint16_t addr)
{
	...
	iic_FRAM_BufferRead((uint8_t*)&FramDataA,(uint16_t)( FramAddrA), sizeof(EnergySavedef));
	iic_FRAM_BufferRead((uint8_t*)&FramDataB,(uint16_t)( FramAddrB), sizeof(EnergySavedef));
	/*A setor crc */
	if(FramDataA.Crc == crc8_ccitt(0xff, &FramDataA, offsetof(EnergySavedef,Crc)) && (FramDataA.num >= FramDataB.num || FramDataB.Crc != crc8_ccitt(0xff, &FramDataB, offsetof(EnergySavedef,Crc))))
	{
		...
		iic_FRAM_BufferWrite((uint8_t*)&EnergData,(uint16_t)( FramAddrB), sizeof(EnergySavedef));
		iic_FRAM_BufferRead((uint8_t*)&FramDataB,(uint16_t)( FramAddrB), sizeof(EnergySavedef));
		if(FramDataB.Crc== EnergData.Crc)
		{
			iic_FRAM_BufferWrite((uint8_t*)&EnergData,(uint16_t)( FramAddrA), sizeof(EnergySavedef));
		}
		else
		{
			iic_FRAM_BufferWrite((uint8_t*)&EnergData,(uint16_t)( FramAddrB), sizeof(EnergySavedef));
	...
	else
	{
		...
		{
			EnergData.Data = EnergyData + FramDataB.Data;
			EnergData.num = FramDataB.num+1;
			EnergData.Crc = crc8_ccitt(0xff,&EnergData,offsetof(EnergySavedef,Crc));
			iic_FRAM_BufferWrite((uint8_t*)&EnergData,(uint16_t)( FramAddrA), sizeof(EnergySavedef));
			iic_FRAM_BufferRead((uint8_t*)&FramDataA,(uint16_t)( FramAddrA), sizeof(EnergySavedef));
			if(FramDataA.Crc== EnergData.Crc)
			{
				iic_FRAM_BufferWrite((uint8_t*)&EnergData,(uint16_t)( FramAddrB), sizeof(EnergySavedef));
			}
			else
...
static BaseType_t prvRN8302bCalibrate( char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString )
{
	...
	RN8302_WriteReg(&hspi1,PHSUA,(uint8_t *)default_RN8302FirmPara.PHSU[0],1);
	RN8302_WriteReg(&hspi1,PHSUB,(uint8_t *)default_RN8302FirmPara.PHSU[1],1);
	RN8302_WriteReg(&hspi1,PHSUC,(uint8_t *)default_RN8302FirmPara.PHSU[2],1);
    HAL_Delay(2000);
    /*相位校准*/
   	for(i = 0; i < READTIME ;i++)
   	{
   		for(j = 0 ; j < 3; j++)
   		{
   			RN8302_ReadReg(&hspi1,0x014+j,(int32_t *)&ReadDat,4);
   			ReadPa[j][i] = ReadDat;
   			for(k=i;k<i+1;k++)
   			{
   				a=k;
   	...
   	for(j = 0;j < 3 ; j++)
   	{
 		...
 		if(abs(PA_HALF_STA -ReadPDat) < (CALIBRATELIMIT*PA_HALF_STA))
 		{
 			...
 			rc = RN8302_WriteReg(&hspi1, PHSUA + j, (uint8_t *)&(RN8302FirmPara.PHSU[j]), 1);
 			if(rc == ERROR)
 			{
    ...
    /*电压电流校准*/
   	...
   	/*清零校验值*/
   	ReadDat = 0;
  	RN8302_WriteReg(&hspi1, GSUA, (uint8_t*)&ReadDat, 2);
  	RN8302_WriteReg(&hspi1, GSUB, (uint8_t*)&ReadDat, 2);
  	RN8302_WriteReg(&hspi1, GSUC, (uint8_t*)&ReadDat, 2);

  	RN8302_WriteReg(&hspi1, GSIA, (uint8_t*)&ReadDat, 2);
  	RN8302_WriteReg(&hspi1, GSIB, (uint8_t*)&ReadDat, 2);
  	RN8302_WriteReg(&hspi1, GSIC, (uint8_t*)&ReadDat, 2);
  	RN8302_WriteReg(&hspi1, GSIN, (uint8_t*)&ReadDat, 2);
    HAL_Delay(2000);
  	for(i = 0; i < READTIME ;i++)
  	{
  		for(j = 0 ; j < 3; j++)
  		{
  			RN8302_ReadReg(&hspi1,0x007+j,(int32_t *)&ReadDat,4);
  			ReadVa[j][i] = ReadDat;
  			for(k=i;k<i+1;k++)
  			{
  				a=k;
  		...
  		for(j = 0 ; j < 3; j++)
  		{
  			RN8302_ReadReg(&hspi1,0x00b+j, (int32_t *)&ReadDat,4);
  			ReadIa[j][i] = ReadDat;
			for(k=i;k<i+1;k++)
				{
					a=k;
  	...
  	for(j = 0 ; j < 3; j++)
  	{
		...
		ReadIDat = ReadIDat/(READTIME-2);
  		...
  		RN8302FirmPara.VGain[j] =(uint16_t)((Standard_Voltage/(float)ReadVDat-1)*(32768));
  		/*读取功率，确保功率因数为1*/
  		RN8302_ReadReg(&hspi1,0x021+j, (int32_t *)&ReadDat,3);
  		if(RN8302FirmPara.VGain[j] < CALIBRATELIMIT *32768 && ReadDat >(1- CALIBRATELIMIT)*8388608)
  		{
  			rc = RN8302_WriteReg(&hspi1, GSUA + j, (uint8_t *)&(RN8302FirmPara.VGain[j]), 2);
  			if(rc == ERROR)
  	  		{
  	  			strncpy(pcWriteBuffer,"RN8302 Write error\r\n",xWriteBufferLen);
  	  			taskEXIT_CRITICAL();
  		...
  		RN8302FirmPara.IGain[j] =(uint16_t)( (Standard_Current/(float)ReadIDat-1)*(32768));
  		if(RN8302FirmPara.IGain[j] < CALIBRATELIMIT *32768 && ReadDat >(1- CALIBRATELIMIT)*8388608)
  		{
  			rc = RN8302_WriteReg(&hspi1, GSIA + j, (uint8_t *)&(RN8302FirmPara.IGain[j]), 2);
			if(rc == ERROR)
			{
				strncpy(pcWriteBuffer,"RN8302 Write error\r\n",xWriteBufferLen);
				taskEXIT_CRITICAL();
...
static BaseType_t prvReadElecData( char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString )
{
	BaseType_t xReturn = pdFALSE;
	static counter=0;
	float *Rn8302DataTab = usRegInputBuf;
	snprintf(pcWriteBuffer,xWriteBufferLen,"IDz1=%d.%d \r\n",Rn8302DataTab[counter]);
			;
...
void RN8302B_init(sRN8302FirmParaFile_TypeDef *RN8302FirmPara )
{
	...
	RN8302_WriteReg(&hspi1,PHSUA,(uint8_t *)&RN8302FirmPara->PHSU[0],1);
	RN8302_WriteReg(&hspi1,PHSUB,(uint8_t *)&RN8302FirmPara->PHSU[1],1);
	RN8302_WriteReg(&hspi1,PHSUC,(uint8_t *)&RN8302FirmPara->PHSU[2],1);
...
void rn8302b_poll(void)
{
  ...
  ErrorStatus ReadSta;
 // float *Rn8302DataTab = usRegInputBuf;
  int32_t *Rn8302DataTab  =  usRegInputBuf;
  ...
  {
	  /*时间未到，跳过电能数据读取*/
	  if(Rn8302_Const[i].DataHandle == energy && ReadCounter < RN8302_ENERGY_NUM)
	  {
		  continue;
	  }

	  ReadSta=RN8302_ReadReg(&hspi1,(uint16_t)Rn8302_Const[i].Address,&ReadData,Rn8302_Const[i].Size);
      if(ReadSta == ERROR)
    	  return ;
